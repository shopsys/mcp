...
class AllowedDatabaseColumnsProvider
{
     ...
     */
    public function getAllowedColumnsSetIndexedByTableNames(array $requestedTableNames): array
    {
        return $this->getAllowedColumnsSetIndexedByClassMetadata(
            $this->allowedDatabaseTablesProvider->getAllowedClassMetadataByTableNames($requestedTableNames),
        );
    }
    ...
    {
        return $this->getAllowedColumnsSetIndexedByClassMetadata(
            $this->allowedDatabaseTablesProvider->getAllAllowedClassMetadataByTableNames(),
        );
    }

    /**
     * @param array<string, \Doctrine\ORM\Mapping\ClassMetadata> $allowedClassMetadataByTableNames
     * @return array<string, array<string, bool>>
     */
    protected function getAllowedColumnsSetIndexedByClassMetadata(array $allowedClassMetadataByTableNames): array
    {
        $allowedColumnsSetIndexedByTableNames = [];

        foreach ($allowedClassMetadataByTableNames as $tableName => $classMetadata) {
    ...
    protected function isColumnExposed(ReflectionProperty $property, array $classLevelColumnExposureByFieldNames): bool
    {
        ...
        if ($asMcpColumnAttributes !== []) {
            return $asMcpColumnAttributes[0]->newInstance()->exposed;
        }
